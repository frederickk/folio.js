...
(function(){var J="undefined"!==typeof global&&null!==global?global:this,m={body:document.body,html:document.documentElement,F3D:{},FIO:{},FTime:{}};paper.install(window);PaperScope.inject({enumerable:!0,print:function(f){console.log(f)},println:function(f){console.log(f);console.log("\n")},boolToInt:function(f){return f?1:0},numToBool:function(f){return 0!==f?!0:!1},getType:function(f){return f instanceof Point?"Point":f instanceof Size?"Size":f instanceof Rectangle?"Rectangle":f instanceof Group?
"Group":f instanceof Raster?"Raster":f instanceof PlacedSymbol?"PlacedSymbol":f instanceof Path?"Path":f instanceof CompoundPath?"CompoundPath":f instanceof Symbol?"Symbol":f instanceof TextItem?"TextItem":Object.prototype.toString.call(f).split(/\W/)[2]},findByName:function(f,a){for(var b,c=0;c<f.length&&(b=f[c],b.name!==a);c++);return b},findById:function(f,a){for(var b,c=0;c<f.length&&(b=f[c],b.id!==a);c++);return b},forEach:function(f,a){for(var b=0,c=f.length;b<c;b++)if(!1===a(f[b],b))return!1;
return!0},clear:function(f,a){var b,c=function(){};2===arguments.length?(b=f,c=a):1===arguments.length&&(b=f);var d;if(void 0===b)for(b=0;b<paper.projects.length;b++)for(var e=0;e<paper.project.layers.length;e++)d=paper.project.layers[e],d.removeChildren(),c(d);else d=paper.project.layers[b],d.removeChildren(),c(d);return d}});PaperScope.inject({enumerable:!0,random:function(f,a){void 0===a&&(a=f,f=0);return f+Math.random()*(a-f)},randomInt:function(f,a){return parseInt(paper.random(f,a))},randomBias:function(f,
a,b){var c=[90,9,4,2.33,1.5,1,.66,.43,.25,.11,.01];b=10*Math.max(0,Math.min(b,1));var d=parseInt(Math.floor(b)),e=c[d];10>b&&(e+=(c[d+1]-e)*(b-d));return Math.pow(Math.random(),e)*(a-f)+f},clamp:function(f,a,b){return f<a?a:f>b?b:f},normalize:function(f,a,b){return(f-a)/(b-a)},map:function(f,a,b,c,d){return c+(f-a)/(b-a)*(d-c)},round:function(f,a){var b=Math.pow(10,a);return Math.round(f*b)/b},roundMultiple:function(f,a){return Math.floor(f/a)*a+a},snap:function(f,a,b){b=b||Math.round;return paper.round(a*
b(f/a),2)},interpolate:function(f,a,b){return f+(a-f)*b},divisor:function(f){for(var a=[],b=[],c=Math.floor(Math.sqrt(f)),d=1;d<=c;d++)0===f%d&&(a.push(d),d*d!==f&&b.push(f/d));b.reverse();return a.concat(b)},degrees:function(f){return 180/Math.PI*f},radians:function(f){return Math.PI/180*f},sec:function(f){return 1/Math.cos(f)},cosec:function(f){return 1/Math.sin(f)},slope:function(f,a){var b=a*Math.cos(paper.radians(f)),c=a*Math.sin(paper.radians(f));return new Point(b,c)},slopeRatio:function(f,
a){return(a.y-f.y)/(a.x-f.x)},slopeAngle:function(f){return paper.degrees(Math.atan(f.x/f.y))},sq:function(f){return f*f},getCommonTangents:function(f,a){var b=a.position.x-f.position.x,c=a.position.y-f.position.y,d=Math.sqrt(f.bounds.size.getArea()),e=Math.sqrt(a.bounds.size.getArea()),d=d/2,e=e/2,g=f.position.getDistance(a.position);if(!(g<=Math.abs(e-d))){var b=Math.atan2(c,b),l=Math.acos((d-e)/g),g=new Point(f.position.x+d*Math.cos(b+l),f.position.y+d*Math.sin(b+l)),c=new Point(a.position.x+e*
Math.cos(b+l),a.position.y+e*Math.sin(b+l)),d=new Point(f.position.x+d*Math.cos(b-l),f.position.y+d*Math.sin(b-l)),e=new Point(a.position.x+e*Math.cos(b-l),a.position.y+e*Math.sin(b-l));return[g,c,e,d]}},reverser:function(f,a,b){f=f||0;a=a||100;b=b||1;var c=0===f&&100===a?100:1,d=!1,e=0;return function(){e=d?e+b:e-b;e<f&&!d?d=!0:e>a&&d&&(d=!1);return e/c}}});paper.Color.inject({data:null,toInt:function(){var f=this.red,f=f<<8|this.green;return f=f<<8|this.blue},toHex:function(){function f(a){a=a.toString(16);
return 1===a.length?"0"+a:a}return"#"+f(255*this.red)+f(255*this.green)+f(255*this.blue)},desaturate:function(f){var a=new Color(this);a.saturation=paper.clamp(this.saturation-this.saturation*f,0,1);a.setType(this.type);return a},saturate:function(f){var a=new Color(this);a.saturation=paper.clamp(this.saturation+this.saturation*f,0,1);a.setType(this.type);return a},darken:function(f){var a=new Color(this);a.lightness=paper.clamp(this.lightness-this.lightness*f,0,1);a.setType(this.type);return a},
dim:function(f){var a=new Color(this);a.brightness=paper.clamp(this.brightness-this.brightness*f,0,1);a.setType(this.type);return a},lighten:function(f){var a=new Color(this);a.lightness=paper.clamp(this.lightness+this.lightness*f,0,1);a.setType(this.type);return a},brighten:function(f){var a=new Color(this);a.saturation=paper.clamp(this.saturation-this.saturation*f,0,1);a.brightness=paper.clamp(this.brightness+this.brightness*f,0,1);a.setType(this.type);return a},contrast:function(f){var a=new Color(this);
a.setType(this.type);return.5>a.lightness?a.darken(f):a.lighten(f)},invert:function(){var f=new Color(this);f.hue+=180;f.setType(this.type);return f},rotate:function(f){var a=new Color(this);a.hue+=f;a.setType(this.type);return a},interpolate:function(f,a,b){var c=new Color(this),d,e;if("number"===typeof a)for(d=f.getComponents(),e=0;e<c._components.length;e++)c._components[e]=paper.interpolate(c._components[e],d[e],a);else for(f=f.getComponents(),d=a.getComponents(),e=0;e<c._components.length;e++)c._components[e]=
paper.interpolate(f[e],d[e],b);c.setType(this.type);return c},statics:{random:function(f){var a;if("String"===paper.getType(f))return a=f.substring(1),new Color.random(parseInt(a.slice(0,2),16)/255,parseInt(a.slice(2,4),16)/255,parseInt(a.slice(4,6),16)/255);if("Object"===paper.getType(f)){a={};for(var b in f)"Array"===paper.getType(f[b])?a[b]=paper.random(f[b][0],f[b][1]):a[b]=paper.random(0,f[b])}else{a=[];b=0<arguments.length?arguments.length:4;for(var c=0;c<b;c++)"Array"===paper.getType(arguments[c])?
a.push(paper.random(arguments[c][0],arguments[c][1])):"Number"===paper.getType(arguments[c])?a.push(paper.random(0,arguments[c])):a.push(paper.random(1))}b=new Color(a);b.data="hex"===f?a:null;b.alpha=1;b.setType(1<a[0]?"hsb":"rgb");return b},integer:function(f){return new Color(f>>16&255,f>>8&255,f&255)},bytes:function(f,a,b,c){return 4===arguments.length?new Color(f/255,a/255||f/255,b/255||f/255,c/255||1):new Color(f/255,a/255||f/255,b/255||f/255)}}});PaperScope.inject({enumerable:!0,PIXEL_TO_MM:.352777778,
MM_TO_PIXEL:2.83464567,POINT_TO_MM:.352777778,MM_TO_POINT:2.83464567,PIXEL_TO_CM:.0352777778,CM_TO_PIXEL:28.3464567,POINT_TO_CM:.0352777778,CM_TO_POINT:28.3464567,PIXEL_TO_INCH:.0138888889,INCH_TO_PIXEL:72,POINT_TO_INCH:.0138888889,INCH_TO_POINT:72,PIXEL_TO_PICA:.0833333333,PICA_TO_PIXEL:12,POINT_TO_PICA:.0833333333,PICA_TO_POINT:12});paper.Item.inject({enumerable:!0,_prevAngle:0,_prevPosition:{x:0,y:0},_prevHor:1,_prevVer:1,snap:function(f,a){this.position.snap(f,a)},getDistanceToCenter:function(){if(void 0!==
this._position){var f=this._position.x-view.bounds.center.x,a=this._position.y-view.bounds.center.y;return f*f+a*a+1}},toGroup:function(){return"CompoundPath"===paper.getType(this)?new Group(this.children):this},setRotation:function(f,a){a=a||this.point;this.rotate(-(f-this._prevAngle),a);this._prevAngle=f;return this},setTranslation:function(f){f=new Point(f);this.translate(new Point(f.x-this._prevPosition.x,f.y-this._prevPosition.y));this._prevPosition=f;return this},setScaling:function(f,a,b){var c,
d,e;if(0!==arguments.length)return 2===arguments.length&&(c=f,a instanceof Point?(d=c,e=a):(d=a,e=this.position)),3===arguments.length&&(c=f,d=a,e=b),this.scale(1+(c-this._prevHor),1+(d-this._prevVer),e),this._prevHor=c,this._prevVer=d,this}});paper.Path.inject({getCentroid:function(){for(var f=new Point(0,0),a=0,b,c=[],d=0;d<this._segments.length-1;++d)c[0]=this._segments[d].point,c[1]=this._segments[d+1].point,b=c[0].x*c[1].y-c[1].x*c[0].y,a+=b,f.x+=(c[0].x+c[1].x)*b,f.y+=(c[0].y+c[1].y)*b;c[0]=
this._segments[d].point;c[1]=this._segments[0].point;b=c[0].x*c[1].y-c[1].x*c[0].y;f.x+=(c[0].x+c[1].x)*b;f.y+=(c[0].y+c[1].y)*b;a=.5*(a+b);f.x/=6*a;f.y/=6*a;return f},getCircumcenter:function(){for(var f=this._segments.length,a=[],b=[],c,d,e,g=0;g<f;g++){c=g+1>=f?0:g+1;d=g+2>=f?1:g+2;e=this._segments[g].point;c=this._segments[c].point;d=this._segments[d].point;var l=c.x-e.x,h=c.y-e.y,k=d.x-e.x,p=d.y-e.y,t=l*(e.x+c.x)+h*(e.y+c.y),n=k*(e.x+d.x)+p*(e.y+d.y),r=2*(l*(d.y-c.y)-h*(d.x-c.x));Math.abs(r)<
Numerical.EPSILON?(l=[e.x,c.x,d.x],e=[e.y,c.y,d.y],c=l.min(),d=e.min(),h=l.max(),k=e.max(),e=new Point((l[c]+l[h])/2,(e[d]+e[k])/2)):e=new Point((p*t-h*n)/r,(l*n-k*t)/r);a.push(e.x);b.push(e.y)}return new Point(a.median(),b.median())},getCircumcircle:function(){function f(b){var d=b.point;b=d.x-a.getCircumcenter().x;d=d.y-a.getCircumcenter().y;return Math.sqrt(b*b+d*d)}var a=this,b=0,b=this._segments.slice();b.sort(function(a,b){return f(a)-f(b)});b=f(b[b.length-1])+f(b[b.length-2]);b/=2;return Shape.Circle(a.getCircumcenter(),
b)},getIncircle:function(){for(var f=Number.MAX_VALUE,a=0;a<this._segments.length;a++){var b=this._segments[a].point;b.getDistance(this.getCentroid())<f&&(f=b.getDistance(this.getCentroid()))}return Shape.Circle(this.getCentroid(),f)},statics:new function(){return{FArrow:function(f,a,b){var c=new Path.Line(f,a);b=b||new Size(.381924*f.getDistance(a),.381924*f.getDistance(a));a=Math.atan2(f.x-a.x,a.y-f.y);var d=[];d[0]=new Path.Line(new Point(0,0),new Point(-b.width,-b.height));d[1]=new Path.Line(new Point(0,
0),new Point(b.width,-b.height));for(b=0;b<d.length;b++)d[b].rotate(180+paper.degrees(a),new Point(0,0)),d[b].translate(f);f=new Group([c,d[0],d[1]]);f.name="arrow";return f},FBubble:function(f,a,b,c){var d=new Path;d.name="bubble";b=b||defaultFBubbleTagSize;10>a.width&&(a.width=10,b=new Size(10,10));c=c||"RANDOM";d.add(new Point(0,0));var e=180,e=new Point(a.height/2+Math.cos(paper.radians(e))*a.height,a.height/2+Math.sin(paper.radians(e))*a.height);d.arcTo(e,new Point(0,a.height));e=paper.randomInt(0,
a.width-b.width);d.add(new Point(e,a.height));c="LEFT"===c?e:"CENTER"===c?e+b.width/2:"RIGHT"===c?e+b.width:paper.randomInt(e,e+b.width);d.add(new Point(c,a.height+b.height));d.add(new Point(e+b.width,a.height));d.add(new Point(a.width,a.height));e=0;new Point(a.height/2+Math.cos(paper.radians(e))*(a.height/2),a.height/2+Math.sin(paper.radians(e))*(a.height/2));d.arcTo(new Point(a.width,0),!1);d.closed=!0;d.position=new Point(f.x,f.y+b.height/2);return d},FChain:function(f,a,b,c){"Point"===paper.getType(f)?
(f=new Path.Circle(f,a),c=new Path.Circle(b,c)):c=a;a=paper.getCommonTangents(f,c);b=new Path;if(null!==a){b.name="chain";b.add(a[0]);b.add(a[1]);angle=c.position.x>f.position.x?0:c.position.y<f.position.y?-90:c.position.y>f.position.y?90:180;var d=new Point(c.position.x+Math.cos(paper.radians(angle))*(c.bounds.width/2),c.position.y+Math.sin(paper.radians(angle))*(c.bounds.height/2));b.arcTo(d,a[2]);b.add(a[2]);b.add(a[3]);angle=f.position.x>c.position.x?0:f.position.y<c.position.y?-90:f.position.y>
c.position.y?90:180;f=new Point(f.position.x+Math.cos(paper.radians(angle))*(f.bounds.width/2),f.position.y+Math.sin(paper.radians(angle))*(f.bounds.height/2));b.arcTo(f,a[0]);b.closed=!0}return b},FCross:function(f,a,b,c){b=b||1;c=c||"LINE";var d,e;"LINE"===c?(d=new Path.Line(f.x+a.width,f.y-a.height,f.x-a.width,f.y+a.height),d.strokeWidth=b,e=new Path.Line(f.x+a.width,f.y+a.height,f.x-a.width,f.y-a.height),e.strokeWidth=b):"SHARP"===c&&(d=new Path,d.add(new Point(f.x+a.width,f.y-a.height)),d.add(new Point(f.x+
a.width,f.y-a.height+b/2)),d.add(new Point(f.x-a.width+b/2,f.y+a.height)),d.add(new Point(f.x-a.width,f.y+a.height)),d.add(new Point(f.x-a.width,f.y+a.height-b/2)),d.add(new Point(f.x+a.width-b/2,f.y-a.height)),d.closed=!0,e=new Path,e.add(new Point(f.x-a.width,f.y-a.height)),e.add(new Point(f.x-a.width+b/2,f.y-a.height)),e.add(new Point(f.x+a.width,f.y+a.height-b/2)),e.add(new Point(f.x+a.width,f.y+a.height)),e.add(new Point(f.x+a.width-b/2,f.y+a.height)),e.add(new Point(f.x-a.width,f.y-a.height+
b/2)),e.closed=!0);f=new Group([d,e]);f.name="cross";return f},FDrip:function(f,a){var b=new Path;b.name="drop";b.add(new Segment(new Point(-.01,.01),new Point(0,-.0055078),new Point(0,.643042)));b.add(new Segment(new Point(-.65,1.6381104),new Point(0,-.6109619),new Point(0,.3694434)));b.add(new Segment(new Point(0,2.31),new Point(-.3578369,0),new Point(.3578369,0)));b.add(new Segment(new Point(.65,1.6381104),new Point(0,.3694434),new Point(0,-.6109619)));b.add(new Segment(new Point(.01,.01),new Point(0,
.643042),new Point(0,-.0055078)));b.add(new Segment(new Point(0,-0),new Point(.0055078,0),new Point(-.0055078,0)));b.closed=!0;b.position=f;"Size"===typeof a?b.scale(a.width,a.height):b.scale(a);return b},FTriangle:function(f,a,b){var c;if(3===arguments.length)c=new Path,c.add(f),c.add(a),c.add(b);else if(2===arguments.length)c=new Path.RegularPolygon(f,3,a);else return;c.closed=!0;c.name="triangle";return c}}}});paper.Point.inject({name:null,data:{},snap:function(f,a){a=void 0!==a?a:{grid:!0,isometric:!1};
f="Size"===f.type?f:new Size(f,f);var b,c;!0===a.isometric?(b=Math.round(this.y/(16*f.height)-this.x/(32*f.width)),c=Math.round(this.y/(16*f.height)+this.x/(16*f.width)),this.x=(c-b)/2*32*f.width,this.y=(c+b)/2*16*f.height):(b=Math.round(this.y/f.height-this.x/f.width),c=Math.round(this.y/f.height+this.x/f.width),this.x=(c-b)/2*f.width,this.y=(c+b)/2*f.height);return this},interpolateTo:function(f,a){return new Point(this.x+(f.x-this.x)*a,this.y+(f.y-this.y)*a)},invertX:function(){this.x=view.bounds.width-
this.x;return this},invertY:function(){this.y=view.bounds.height-this.y;return this},getDistanceToCenter:function(){return this.getDistance(view.bounds.center)},getMid:function(f){return new Point((this.x+f.x)/2,(this.y+f.y)/2)},getPerpendicularBisector:function(f,a){var b=new Path.Line(this,f);b.setRotation(90);a=a||1;"fit"===a&&(a=view.size.getArea()/b.bounds.size.getArea());b.setScaling(a,a);var c={start:b.segments[0].point,mid:b.position,end:b.segments[1].point};b.remove();return c},getSlope:function(f){return paper.slopeRatio(this,
f)},getHeading:function(){return-1*Math.atan2(-this.y,this.x)},getAngle:function(f){f=f||new Point(0,0);return Math.atan2(f.y-this.y,f.x-this.x)},norm:function(f,a){this.x=paper.normalize(this.x,f.x,a.x);this.y=paper.normalize(this.y,f.y,a.y);return this},magSq:function(){return this.x*this.x+this.y*this.y},statics:{interpolateTo:function(f,a,b){return f.interpolateTo(a,b)},invertX:function(f){f=new Point(f);return f.invertX()},invertY:function(f){f=new Point(f);return f.invertY()},random:function(f,
a){var b="array"===typeof f?paper.random(f[0],f[1]):paper.random(f),c="array"===typeof f?paper.random(a[0],a[1]):paper.random(a);return new Point(b,c)}}});paper.Size.inject({getArea:function(){return this.width*this.height},getCircumarea:function(){var f=this.getCircumradius();return Math.PI*f*f},getIncirclearea:function(){var f=this.getIncircleradius();return Math.PI*f*f},getCircumradius:function(){var f=this.width,a=this.height;return Math.sqrt(f*f+a*a)/2},getIncircleradius:function(){return this.width<
this.height?this.width/2:this.height/2},getSlopeAngle:function(){return Math.atan(this.width/this.height)}});paper.TextItem.inject({trimToFit:function(){this.content=this.visibleRange.content.trim();return this}});Base.extend({_class:"Pattern",initialize:function a(b){this._id=a._id=(a._id||0)+1;this._path||this.setPath(b.path||null);this._min||this.setMin(b.min||0);this._max||this.setMax(b.max||100);this.setDensity(b.density||1);this.setStrokeWidth(b.strokeWidth||null);this.setStrokeColor(b.strokeColor||
null);this.setFillColor(b.FillColor||null)},_serialize:function(a,b){return Base.serialize([this._path,this._min,this._max,this._density,this._strokeWidth,this._strokeColor,this._fillColor],a,!0,b)},_changed:function(){this._owner&&this._owner._changed(65)},setPath:function(a){this._path=a;this._changed()},setMin:function(a){this._min=a;this._changed()},setMax:function(a){this._max=a;this._changed()},setDensity:function(a){this._density=Math.round(this._max*a)<this._min?this._min:Math.round(this._max*
a);this._changed()},setStrokeWidth:function(a){this._strokeColor=a;this._changed()},setStrokeColor:function(a){this._strokeColor=Color.read(arguments);this._strokeColor===a&&(this._strokeColor=a.clone());this._strokeColor._owner=this;this._changed()},setFillColor:function(a){this._fillColor=Color.read(arguments);this._fillColor===a&&(this._fillColor=a.clone());this._fillColor._owner=this;this._changed()}}).inject({statics:new function(){return{FPolkaDot:function(){},FHatch:function(){var a=Base.readNamed(arguments);
console.log("FHatch OPTIONS",a);console.log("this._strokeColor",a.strokeColor);console.log("this._min",a.min);return new Group}}}});m.FTime={FDate:{},FStopwatch:{},FStepper:{},Ease:{}};m.FTime.Ease=function(){return{linear:function(a){return a},inQuad:function(a){return a*a},outQuad:function(a){return a*(2-a)},inOutQuad:function(a){return.5>a?2*a*a:-1+(4-2*a)*a},inCubic:function(a){return a*a*a},outCubic:function(a){return--a*a*a+1},inOutCubic:function(a){return.5>a?4*a*a*a:(a-1)*(2*a-2)*(2*a-2)+
1},inQuart:function(a){return a*a*a*a},outQuart:function(a){return 1- --a*a*a*a},inOutQuart:function(a){return.5>a?8*a*a*a*a:1-8*--a*a*a*a},inQuint:function(a){return a*a*a*a*a},outQuint:function(a){return 1+--a*a*a*a*a},inOutQuint:function(a){return.5>a?16*a*a*a*a*a:1+16*--a*a*a*a*a},inSine:function(a){return-1*Math.cos(Math.PI/2*a)+1},outSine:function(a){return 1*Math.sin(Math.PI/2*a)},inOutSine:function(a){return-.5*(Math.cos(Math.PI*a)-1)},inExpo:function(a){return 1*Math.pow(2,10*(a-1))},outExpo:function(a){return 1*
(-Math.pow(2,-10*a)+1)},inOutExpo:function(a){a/=.5;return 1>a?.5*Math.pow(2,10*(a-1)):.5*(-Math.pow(2,-10*a)+2)},inCirc:function(a){return-1*(Math.sqrt(1-a*a)-1)},outCirc:function(a){a--;return 1*Math.sqrt(1-a*a)},inOutCirc:function(a){a/=.5;if(1>a)return-.5*(Math.sqrt(1-a*a)-1);a-=2;return.5*(Math.sqrt(1-a*a)+1)},spline:function(a,b){function c(a,b){return 1-3*b+3*a}function d(a,b){return 3*b-6*a}function e(a){return 3*a}function g(a,b,n){return((new c(b,n)*a+new d(b,n))*a+new e(b))*a}function l(a,
b,n){return 3*new c(b,n)*a*a+2*new d(b,n)*a+new e(b)}function h(a){for(var b=a,c=0;4>c;++c){var d=new l(b,k[0],k[2]);if(0===d)break;var e=new g(b,k[0],k[2])-a,b=b-e/d}return b}var k;a instanceof Array?k=a:(a=a.normalize(),b=b.normalize(),k=[a.x,a.y,b.x,b.y]);return{get:function(a){return k[0]===k[1]&&k[2]===k[3]?a:new g(new h(a),k[1],k[3])}}},values:{ease:[.25,.1,.25,1],linear:[0,0,1,1],out:[0,0,.58,1],inOut:[.42,0,.58,1]}}};m.FTime.FDate=function(){var a=new Date,b="Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "),
c=function(a){1===a.length&&(a="0"+a);return a},d=function(){void 0===a&&(a=new Date);return String(a.getFullYear())},e=function(){void 0===a&&(a=new Date);return String(a.getMonth())},g=function(){void 0===a&&(a=new Date);return String(a.getDate())},l=function(){void 0===a&&(a=new Date);var b=String(a.getHours());return c(b)},h=function(){void 0===a&&(a=new Date);var b=String(a.getMinutes());return c(b)},k=function(){void 0===a&&(a=new Date);var b=String(a.getSeconds());return c(b)},p=function(a,
b,c){void 0===b&&void 0===c&&(a=t(a)[0],b=t(a)[1],c=t(a)[2]);return parseInt(36E5*a+6E4*b+1E3*c)},t=function(a){a=a.split(":");return[a[0],a[1],a[2]]};return{year:d,month:e,day:g,weekday:function(){void 0===a&&(a=new Date);return String(b[a.getDay()])},hour:l,minute:h,second:k,date:function(){return d()+e()+g()},now:function(a){a=a||[!0,!0,!0];var b="";a[0]&&(b+=l());a[0]&&a[1]&&(b+=":");a[1]&&(b+=h());a[1]&&a[2]&&(b+=":");a[2]&&(b+=k());return b},nowMilliseconds:function(){return p(l(),h(),k())},
add:function(b,c,d,e){return a+(24*b+60*c+60*d+1E3*e)},sub:function(b,c,d,e){return a-(24*b+60*c+60*d+1E3*e)},set:function(a,b,c,d){time=new Date;time.setTime(24*a+60*b+60*c+1E3*d);return time},get:function(a,b){var c=b||[!0,!0,!0],d=parseInt(a/1E3%60),e=parseInt(a/1E3/60%60),u=parseInt(a/1E3/60/60%24),t="";c[0]&&(t+=10>u?"0"+u:""+u);c[0]&&c[1]&&(t+=":");c[1]&&(t+=10>e?"0"+e:""+e);c[1]&&c[2]&&(t+=":");c[2]&&(t+=10>d?"0"+d:""+d);return t},toMillsecond:p,toArray:t}};m.FTime.FStepper=function(){function a(a){b=
a;b/=1E3}var b=1E3,c=0,d=0,e=!1,g=!1,l=!1,h=1,k=0,p=function(){g=e=!0;l=!1},t=function(){l=e=!0;g=!1};return{delta:function(){return h},counter:function(){return k},toggle:function(){0===d?(d=1,t()):(d=0,p());return d},update:function(a){e&&(e=!1,c=g?paper.round(a+(1-h)*b,3):paper.round(a+h*b,3),c<=a&&(g?(g=!1,h=1):(l=!1,h=0)));g?(h=paper.round(1-(c-a)/b,3),a===c&&(g=!1,h=1,k++)):l&&(h=paper.round((c-a)/b,3),a===c&&(g=!1,h=0,k++))},stepIn:p,stepOut:t,isIn:function(){return g},isOut:function(){return l},
...
Setup();"function"===typeof Draw&&Draw();view.draw()}m.body=m.body||document.body;m.html=m.html||document.documentElement;var b=document.getElementsByTagName("canvas"),c;0===b.length?(b=document.createElement("div"),b.id="container",b.style.position="absolute",b.style.top=0,b.style.left=0,b.style.width="100%",b.style.height="100%",m.body.appendChild(b),c=document.createElement("canvas"),c.id="canvas",c.resize=!0,c.style.backgroundColor="transparent",c.style.webkitUserDrag="none",c.style.webkitUserSelect=
"none",c.style.webkitTapHighlightColor="transparent",b.appendChild(c)):c=b[0];""===c.style.width&&(c.style.width="100%");""===c.style.height&&(c.style.height="100%");c.classList.add("folio-canvas");paper.setup(c);"function"===typeof Setup&&Setup();"function"===typeof Draw&&Draw();(new Group).name="__AnimationGroup";view.onFrame=function(a){"function"===typeof Update&&Update(a);view&&view.update()};view.onResize=function(a){"function"===typeof onResize&&onResize(a)};b=new Tool;b.onMouseUp=function(a){"function"===
typeof onMouseUp&&onMouseUp(a)};b.onMouseDown=function(a){"function"===typeof onMouseDown&&onMouseDown(a)};b.onMouseMove=function(a){"function"===typeof onMouseMove&&onMouseMove(a)};b.onMouseDrag=function(a){"function"===typeof onMouseDrag&&onMouseDrag(a)};b.onKeyDown=function(a){"function"===typeof onKeyDown&&onKeyDown(a)};b.onKeyUp=function(a){"function"===typeof onKeyUp&&onKeyUp(a)};view.draw();view.update();window.addEventListener("resize",a,!1);window.addEventListener("orientationchange",a,!1);
console.log("Folio.js is go!")});J.folio=m}).call(this);
